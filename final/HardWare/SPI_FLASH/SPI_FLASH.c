...
//{
//    spi_flash_buffer_write((uint8_t*)data, addr, size);
//}

//void flash_param_read(uint32_t addr, void* data, uint16_t size)
//{
//    spi_flash_buffer_read((uint8_t*)data, addr, size);
//}
...
void save_flash(float value, uint32_t addr)
{
  spi_flash_sector_erase(addr); //先擦除地址

  spi_flash_buffer_write((uint8_t*)&value, addr, sizeof(float)); //写入数据保存
}
void save_flash_long(long value, uint32_t addr)
{
  spi_flash_sector_erase(addr); //先擦除地址
  spi_flash_buffer_write((uint8_t*)&value, addr, sizeof(long)); //写入数据保存
}
float read_flash(uint32_t addr)
{
  float value;
  spi_flash_buffer_read((uint8_t*)&value, addr, sizeof(float)); //读取数据
  return value;
}
